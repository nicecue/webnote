...
export class MemoStore {
  constructor(initialMemos?: MemoModel[]) {
    if (initialMemos !== null && initialMemos !== undefined) {
  ...
  load = () => {
    ...
    if (jsonMemos !== null && jsonMemos !== undefined) {
  ...
  @observable
  public memos: MemoModel[];

  @action
  search(word: string) {
    return this.memos.filter(memo => memo.title.indexOf(word) >= 0);
  }

  @action
  getMemo(memoId: number) {
    for (let i = 0; i < this.memos.length; i += 1) {
